...
`gemini-3.7-flash` で福よし 10 分（チャンク 60 秒 × 10 本）を流したときの**実測**。

```
入力  20,358 tok × $0.75/M = $0.01527
出力  18,013 tok × $3.75/M = $0.06755   (本文 14,080 + 思考 3,933)
                    合計      $0.08282
              動画 1 時間換算  $0.50
```

**思考トークン（`thoughtsTokenCount`）も出力として課金される。**
思考が出力に占める割合はモデルで大きく変わる。同じ 10 分で 3.7 は 22%、
3.8 は 24% だったが、**3.6 は 62%**（思考 21,461 tok）で額が倍近くなった。
返ってくる字幕の長さだけで見積もると大きく外す。

1 時間あたりで並べると、Gemini は専用 ASR の 2〜3 倍。
...
| AssemblyAI | $0.17 |
| Deepgram | $0.26 |
| **Gemini 3.7 Flash（既定）** | **$0.50** |
| Gemini 3.8 Flash | $0.52 |
| Gemini 3.6 Flash | $0.87 |

**この単価は 2026-12-31 までの割引価格。** 2027-01-01 に入力 $1.50 / 出力 $7.50 へ
戻り、1 時間あたりは倍になる。`traccia/gemini.py` の `PRICING` も年が明けたら直すこと。

3.5 Flash Lite は入力 $0.30 / 出力 $2.50 と安い。単価は変わるので
<https://ai.google.dev/gemini-api/docs/pricing> で確認すること。

### モデルを選ぶ

**既定は `gemini-3.7-flash`。**福よし 10 分（手作業で直した正解 310 行）に対して
3 つを流し、`python -m traccia diag` で測った結果で決めた。

| | 3.6 Flash | **3.7 Flash** | 3.8 Flash |
|---|---|---|---|
| 話者が正解と一致 | 52% | **77%** | 65% |
| 開始のずれ 中央 | -0.70s | **-0.09s** | -0.21s |
| 2 秒以上ずれた行 | 60 件 | 7 件 | **3 件** |
| ずれの塊 | 9 か所 | 1 か所 | 1 か所 |
| 最悪のずれ | -15.6s | -2.7s | **-2.1s** |
| 10 分あたりの実額 | $0.144 | **$0.083** | $0.087 |
| 所要 | 174 秒 | **66 秒** | 75 秒 |

**3.6 の話者 52% は、全行を最頻話者に振ったときの 52% と同じ。**話者分離が
働いていない。時刻も 1:47〜2:03 で 7 行がまとめて 15 秒ずれるなど、塊が 9 か所出た。

3.7 と 3.8 は時刻がほぼ互角で、差が付いたのは話者（77% 対 65%）。手作業の負担は
話者の付け直しの方が重いので 3.7 を既定にした。各 1 回ずつの結果である点に注意。

再現するには次を回す（`--dur` があるので正解のある範囲だけに課金を絞れる）。

```bash
python -m traccia diag resources/福よし_verification2 \
  --model gemini-3.7-flash --dur 600 --chunk 60 --truth <正解の edit.json> --yes
```

**`gemini-3.5-transcribe` は使えない。**JSON モードに対応しておらず
（HTTP 400 `JSON mode is not enabled for this model`）、外して投げると
素の書き起こしが 1 本返るだけで時刻も話者も付かない。

### モデル名に注意

入れ替わりが早く、古い名前は 404 になる（`gemini-2.5-flash` は新規提供終了。
`gemini-3-flash` も一覧から消え、残っているのは `gemini-3-flash-preview` のほう）。
**そのキーで使えるものを一覧できる。**
...
Gemini 3.7 Flash は $0.50/時なので、**1 時間の動画を 20 本で $10** に達する。
2027-01-01 に単価が倍に戻ると、同じ $10 が 10 本ぶんになる。
